...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # ===================================================================================
        #                  Addition for MODIFY LOSS FUNCTION
        # 
        # Calculates the average intersection between attention maps and human fixation maps
        # for every sample of the batch_size. For each layer, loops over all heads and 
        # calculate the average intersection of this layer. This is done for every sample of 
        # the batch and it adds a column of size batch_size to the AVG_INTERSECTION tensor
        #
        # Returns:
        #
        #   - AVG_INTERSECTION: (tensor) [batch_size, n_layers]
        # ===================================================================================
        normalize_tensor = lambda x: (x - torch.min(x)) / (torch.max(x) - torch.min(x))

        def compute_intersection(fixation, attn):
            ''' Compute average intersection for all heads of an Attention layer
            INPUT:
                'fixation': (tensor) [batch_size, 196]
                'attn': (tensor) [batch_size, n_heads=12, 197, 197]

            RETURNS:
                'avg_heads_intersection': (tensor) [batch_size]

            '''
            # print()
            batch_size = attn.shape[0]
            n_heads = attn.shape[1]
            avg_heads_intersection = torch.zeros(batch_size)
            for sample in range(batch_size): # Loop over samples
                fixation_weights = fixation[sample].cpu()
                sum_intersection = 0
                for head, attn_weights in enumerate(attn[sample]): # Loop over attention heads for every sample
                    attn_weights = attn_weights.mean(axis=-2)  # Average over all attention keys
                    attn_weights = attn_weights[1:]  # Skip the [class] token
                    attn_weights = torch.reshape(attn_weights, (14, 14))
                    attn_weights = attn_weights.flatten()
                    attn_weights = normalize_tensor(attn_weights)
                    attn_weights = attn_weights.cpu()

                    intersection = np.dot(fixation_weights, attn_weights) # Calculate intersection as dot product

                    # print(f'head_{head}, intersection: {intersection: 0.2f}')
                    sum_intersection += intersection
                avg_heads_intersection[sample] = sum_intersection / n_heads
                # print(f'avg_heads_intersection for sample_{sample}: {avg_heads_intersection[sample]: 0.2f}')
            return torch.unsqueeze(avg_heads_intersection, 0)

        global AVG_INTERSECTION
        fixation = FIXATION
        intersection = compute_intersection(fixation, attn)

        if AVG_INTERSECTION == None:
            AVG_INTERSECTION = intersection.t()
        else:
            AVG_INTERSECTION = torch.cat((AVG_INTERSECTION, intersection.t()), dim=1)
        
        # print('AVG_INTERSECTION:', AVG_INTERSECTION.shape)
        # print('AVG_INTERSECTION, average of all layers:', torch.mean(AVG_INTERSECTION))
        # ===================================================================================
        #
        # ===================================================================================
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            act_layer: MLP activation layer.block_fn
    ...
    def forward(self, x, fixation):
        # print('fixation inside ViT:', fixation.shape)
        # print('image inside ViT:', x.shape)
        ...
        global AVG_INTERSECTION
        FIXATION = fixation
        AVG_INTERSECTION = None

        x = self.forward_features(x)
        x = self.forward_head(x)
        return x, AVG_INTERSECTION
