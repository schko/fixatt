...
Gaussian matrix is initialized using predefined variable. This matrix used along with the frame dimension 
...
def gaussian(x, sx, y=None, sy=None):
    ...
    for i in range(x):
        for j in range(y):
            M[j, i] = numpy.exp(-1.0 * (((float(i)-xo)**2 /
                                (2*sx*sx)) + ((float(j)-yo)**2/(2*sy*sy))))
